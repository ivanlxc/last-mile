...
async function setup(agents: AgentGateway) {
  ...
  const get = async () =>
    (await service.read("getSession", sid)) as P.SessionProjection;
...
describe("persisted Agent job deadlines", () => {
  for (const mode of ["configured", "defaults", "legacy_gateway"] as const)
    it(`${mode}: public job IDs resolve to the correct creation and explicit retry deadlines`, async () => {
      ...
      await settle();
      requests.length = 0;
      const advice = (await x.command("askAdvisor", {
      ...
      await settle();
      expect(
        await x.service.read("getAdvice", x.sid, advice.job.jobId),
      ).toMatchObject({
      ...
      await settle();
      expect(
        await x.service.read("getEvaluation", x.sid, first.job.jobId),
      ).toMatchObject({
        status: "fallback",
        attemptCount: 1,
      });
      x.clock.time += 4000;
      const retried = await requestEvaluation();
      ...
      x.clock.time += 4000;
      expect((await requestEvaluation()).job.jobId).toBe(first.job.jobId);
