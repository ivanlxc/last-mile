...
原创编排的虚拟乐器试听小样，供确定配乐方向使用。版本目录保留音符编排、MP3 和实测混音数据，后续修改另建版本以便比较。这些文件仍是带淡入淡出的试听母带；下方各版状态记录为当时的历史状态。

**现已接入游戏：** 用户确认后，采用开场 v1、西门 v3、市集 v5、主桥 v5、结尾 v4。运行时副本位于 `client/public/audio/music/`，由共用播放器进行章节过渡和循环交叠；控制方式、采用文件与验证见[章节音乐系统](../../../docs/music-system.md)。接入版检查点：`checkpoint/audio-06-game-integration`。

## 已确认的整体方向

用户要求所有任务关卡的背景音乐都偏深沉、克制，不能滑稽、搞笑或呈现轻快的侦探喜剧感。章节差异通过持续音的层次、和声张力、空间与密度表现；结尾可以相对舒缓、释然。已认可的开场 **Last Light v1** 保持原样。

## v5 · 市集与主桥的段落起伏 · 2026-09-19

用户反馈 v4 市集与主桥稍显平淡。本版在原有深沉音色基础上加入更清晰的旋律发展、渐强渐弱和声部进退。开场 v1、西门 v3、结尾 v4 继续引用原文件。

| 试听 | 时长 | 段落变化 |
| --- | --- | --- |
| [Unsettled Accounts · 市集 v5](v5/market.mp3) | 1:12 | 大提琴短主题两次展开：约 8–22 秒第一次积累，24–30 秒留出安静过渡，32–47 秒第二次展开，随后回落 |
| [The Weight Across the Water · 主桥 v5](v5/bridge.mp3) | 1:17 | 弦乐内声部逐步展开，约 30–34 秒短暂悬停；34 秒后持续渐强，47–51 秒形成高点，最后缓缓退下 |

市集以两轮较小的波动表现信息交错；主桥以更长的上行乐句和逐渐扩展的音区承载最终行程。长音现在有独立的渐强渐弱，安静段落仍保留柔和支撑。第一轮内部试渲染的强弱落差过大，已收窄表达曲线，保留适合低音量阅读线索的背景。

- 最新五段文件索引：[collection.json](v5/collection.json)；v4 市集与主桥完整保留，便于比较。
- 编排和分段说明：`v5/market.score.json`（90 个音符）、`v5/bridge.score.json`（80 个音符）。
- 两段统一至约 −19 LUFS；编码后实测值见 `v5/*.mix.json`，技术检查与 v4 动态对比见 [verification.json](v5/verification.json)。
- `gainAutomation` 是可选的逐轨表达曲线：`[{"beat": 0, "gain": 0.8}, {"beat": 16, "gain": 1.1}]`。拍点严格递增，范围为 0 到总拍数；倍率范围 0–2，且与轨道音量相乘不能超过 1。点间线性插值，首尾保持，空数组或省略时倍率为 1。离线渲染每约 5.3 毫秒更新一次，再进入共用混响；旧谱仍使用原渲染路径。
- 两段仍为独立试听母带，带淡入淡出；尚未接入游戏播放或制作无缝循环。
- 本轮前检查点：`checkpoint/pre-music-dynamics-v5-20260919`；完成版：`checkpoint/audio-05-dynamic-cues`。

重建时使用下方命令格式，将谱面与 MP3 路径替换为 `v5/market` 或 `v5/bridge`；原始 WAV 存放在 `.local-artifacts/music-auditions/`。
