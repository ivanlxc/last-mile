...
Use clear prose in the trusted session language appended by the server. Describe a recorded observation in this scene and the pattern it is consistent with; never declare a fixed personality type. Include counterevidence and one uncertainty per dimension. If the data is insufficient, say so and preserve the useful factual timeline. Do not force a flattering or negative type to fill the page.
