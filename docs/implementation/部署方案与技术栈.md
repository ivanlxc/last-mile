...
核对日期：2026-09-16；本轮目录整理同步了最新的平台偏好，未实施部署。适用范围：团队与评委通过定向分享链接试玩；每名玩家独立进行一局单人游戏。

本文是部署咨询与后续实施方案。未创建云账号、购买资源、发布服务、制作 Docker 镜像或验证云端运行。标为“现有”的内容依据当前代码；标为“建议／待实现”的内容不代表已完成。双语界面的完成范围以本轮本地化验收为准。

## 1. 推荐方案

**当前建议 Render：付费 Web Service、持久磁盘、SQLite、一个应用进程。** 用户随后表达了优先使用 Render 或 Vercel 的偏好；预算与最终平台尚未确认。 页面、API 和 SSE 使用同一个 HTTPS 域名。补好试玩入口口令、独立访客身份、游戏归属校验及 AI 总预算后，再向团队和评委分享链接。

**备选 Railway：常驻 Web Service＋持久卷。** 可采用同样的应用结构。暂不需要把游戏拆成多个服务，也不需要为了首轮试玩更换数据库。
...
现有实现：Advisor 每局最多 30 次实际发送；每个逻辑 job 最多 2 次尝试；Evaluator 按封存／配置绑定重试上限。当前本机已验证 OpenAI `gpt-5.6-luna`、`OPENAI_REASONING_EFFORT=low`，显式配置 Advisor 30 秒／2,500 输出 tokens、Evaluator 45 秒／5,000 输出 tokens；未设置对应四个 `AI_*` 参数时，代码回退默认仍为 8 秒／1,200 tokens 与 20 秒／3,000 tokens。配置项与有限真实测试记录见[真实模型接入验收](真实模型接入验收.md)。格式、引用或明显错语种最多修复一次；失败明确标为离线模板。成功、失败、超时及未知结果均保留记账。

上线还需增加：全站模型并发限制、按活动／访客的调用预算、全局金额或 token 预算及人工停用开关。达到预算时拒绝新的付费发送并明确回退；不能只显示一个预算数字而继续调用。金额估算按所选模型的当前价目乘实际输入／输出用量；本次已验证一个模型，但未完成真实完整局的用量测量，不能据此给出“每局固定价格”。超时后供应商仍可能已经收费，因此供应商账单与本地调用记录需要核对。

OpenAI 的中英文合成测试与有限英文领域运行链路已通过，本机已重启为 OpenAI 模式；这些结果不等于云端验收。上线时仍先用 `MODEL_PROVIDER=offline` 验证部署基础流程，再启用受限真实模型，完成中英文完整局验证。切换语言仍只改变呈现及模型输出语言，不能扩大 AI 能看到的隐藏事实范围。服务器只发送玩家已授权材料；评委自行输入的文字也会进入选定供应商，应在入口简短说明并提示使用游戏内虚构信息。
