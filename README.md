...
一款单人、人机协作决策游戏。玩家护送二十名平民穿越虚构的萨赫尔河谷，在有限调查额度内判断情报、选择提供给 AI 的资料并决定路线；游戏没有总时限。三关、两套隐藏剧本、独立行为复盘可完整运行。当前版本支持完整中文和英文，首次进入默认英文。
...
新任务的调查、已有简报、路线行驶和「Advance time」均立即结算，不再让玩家实际等待剧情中的几十秒或几分钟。调查完成后自动打开报告，路线完成后显示结果并更新地图位置；网络提交期间显示处理状态，失败时保留确认窗口供重试。AI 分析和语音仍按真实请求进度显示状态。

界面的 **Play time** 记录实际游玩用时，无上限；**In-world time** 只累计行动的模拟耗时。阅读、思考、打开地图或 AI 助手都不会推进剧情时间。复盘分别记录两种时间，现阶段不新增时间扣分。实现与验证见[即时行动说明](docs/instant-actions.md)。

## 章节音乐与声音

游戏已接入五段原创背景音乐：开场／简报、西门、市集、主桥和结局，随公开章节自动切换，过渡约 2 秒；同一章节持续播放并交叠衔接。音乐播放器跨页面与地图视图保留，打开情报或切换 2D／3D／Unity 不会重播音乐。

首次点击页面或进入简报后开启声音。点击 **Sound** 可静音、分别调整 **Music** 和 **Ambience** 音量；偏好保存在当前浏览器。朗读时背景音乐自动压低，录音时背景声音静音，页面隐藏时暂停，返回后恢复。浏览器限制播放或文件暂时加载失败时，声音面板会提供播放／重试入口。

配乐随应用静态资源提供，无需额外 API key。采用版本、循环边界与验证说明见[章节音乐系统](docs/music-system.md)；历史试听文件与 Git 检查点均保留。

## 2D 卫星风格地图

二维视图使用现有 Blender 地形的正交俯视参考制作卫星风格底图，保留公开道路、地点和车队位置。支持拖动、滚轮缩放及视野复位；地点可点击或通过键盘选择。底图为虚构场景的静态美术，不是实时侦察影像。素材与投影约定见[地图美术说明](assets/authoring/satellite-map/README.md)。
...
Unity 场景现使用 Blender 导出的模型、材质和独立车队，支持透视镜头、光照阴影与公开车队位置同步。即时行动完成后地图显示结算后的地点，不再按路线耗时播放几十秒的行驶过程。按 F 或画布中的跟随按钮近看车队，Home 返回全景。修改建模可在安装 Blender 后运行 `pnpm build:art`，再运行 `pnpm build:unity`；原始工程与新版工程分别保存在 `assets/authoring/last-mile-v2/` 和 `assets/authoring/last-mile-unity/`。
...
密钥只在服务器内存读取，不进入网页、SQLite业务快照、日志或导出。Advisor 与 Evaluator 使用独立提示词和互不重用的输入；可以共用一个供应商账户。真实模式会在进入场景、上传材料和提问后分析，终局调用独立评价。每局最多 30 次 Advisor 实际发送，每个任务最多 2 次尝试；格式、引用或明显错语种输出最多允许一次修复，仍受剩余额度限制；网络失败不会自动重复收费重试。上面的已验证配置给 Advisor 每次请求 30 秒、2,500 输出 tokens，Evaluator 45 秒、5,000 输出 tokens。未设置四个 `AI_*` 参数时，代码仍回退为 8 秒／1,200 tokens 与 20 秒／3,000 tokens；本轮发现原 8 秒窗口过短，因此接入此模型时应显式填写配置。失败会明确切换到离线模板；实际游玩用时照常记录，剧情时间不因等待 AI 而增加。
...
- 简报页不计时；按“开始护送”后开始累计实际用时，R00 的 30 秒行驶作为剧情时间立即结算并进入第一关。没有任务总时限，可以从容阅读和探索。
...
- 每次调查、溯源、更正都可能消耗新的模拟时间/额度，但报告直接交付；选择路线不会再因这些调查尚未完成而需要等待或取消。
- 可以随时切换二维卫星风格地图/三维沙盘。地图上的车队按服务器位置推进；二维底图不是侦察证据。
...
SQLite 默认保存在 `.last-mile/game.sqlite`，采用 WAL、事务、幂等命令和不可变终局记录。**浏览器刷新不会重置实际游玩用时**，剧情时间仅随行动推进；同一次服务启动中能恢复当前页面。
